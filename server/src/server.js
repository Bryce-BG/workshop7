 ...
 function getUserIdFromToken(authorizationLine) {
   try {
     ...
     if (typeof id === 'string') {
       ...
       // Not a number. Return "", an invalid ID. (not a string)
       return "";
     }
   } catch (e) {
     // Return an invalid ID.
     return -1; //SHOULDN"T THIS BE CHANGED TO RETURN ""???????
   }
 }
...
function getFeedData(user, callback) {
 ...
 app.get('/user/:userid/feed', function(req, res) {
   ...
   console.log("userID is: " + userid);                                         //debug statment (id is correct however for some reason the databse is returning null. probably because mongodb isn't loading the data right)
 ...
 //`POST /feeditem { userId: user, location: location, contents: contents  }` (we can base adding a comment off this)
